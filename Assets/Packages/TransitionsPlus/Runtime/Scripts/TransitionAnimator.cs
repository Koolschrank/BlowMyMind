...
namespace TransitionsPlus {
    ...
    public class TransitionAnimator : MonoBehaviour {
        ...
        [Tooltip("Disables main camera and enables second camera when transition ends")]
        ...
        private void OnEnable() {
            if (playing && progress < 1f && profile != null && profile.duration > 0) {
                startTime = Time.unscaledTime - progress * profile.duration;
            }
        }
...
// inside Editor, the capure/toggle canvas from UpdateMaterialProperties() is delayed on frame to avoid OnValidate errors so we force the refresh here, to ensure camera image is present on frame 0
        ...
        void StartTransition() {
            playing = true;
            // I replaced time with unscaledTime to avoid issues with Time.timeScale
        ...
        void Update() {
            ...
            // I replaced time with unscaledTime to avoid issues with Time.timeScale
            float t = profile.duration > 0 ? Mathf.Clamp01((Time.unscaledTime - startTime) / profile.duration) : 1;
            SetProgress(t);
